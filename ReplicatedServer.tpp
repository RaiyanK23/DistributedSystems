...
//Initially going with a gosip architecture
// template <typename ServerType>
...
void ReplicatedServer<ServerType>::initiateElectionProcess() {
    ...
    electionInProgress = true;  // indicating an election is in progress
    for (const auto& peer : peerInfo) {
    ...
    // checking if any responses were received
    // For simplicity, assume immediate assumption of leadership if no higher IDs are present
